...
def check_difference_channels(next_point,difference_channels,cluster):
    farthest_point = max(cluster, key=lambda p: euclidean_distance(p, next_point))
    if abs(farthest_point[0] - next_point[0]) + abs(farthest_point[1] - next_point[1]) + abs(farthest_point[2] - next_point[2]) < difference_channels:
        return True
    else:
        return False
...
def segmentation(image_hsv,difference_channels,difference_in_density,radius):
    ...
    for point in raw_points:
        ...
        count_points_in_cluster = count_points_in_cluster + point_density
        for next_point in raw_points:
            if check_distance(ast.literal_eval(point),ast.literal_eval(next_point),radius) and check_difference_channels(ast.literal_eval(next_point),difference_channels,cluster) \
            ...
            elif check_distance(ast.literal_eval(point),ast.literal_eval(next_point),radius) and check_difference_channels(ast.literal_eval(next_point),difference_channels,cluster) \
...
clusters = segmentation(image_hsv, 50, 25, 35)
